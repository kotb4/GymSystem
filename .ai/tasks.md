...
- Phase 5 (done, NOT yet committed): merged attendance screens. `/checkin` (fast barcode/card) + `/reception` (search + status review) → single `/attendance` page with permission-aware tabs. Extracted `FastCheckInTab` + `ReceptionTab` into `src/components/attendance/`; `checkin-page.tsx`/`reception-page.tsx` re-export (legacy routes preserved). Removed `/checkin`+`/reception` from sidebar, added single `/attendance` (ScanLine, `daily` group). Added i18n `nav.attendance`, `nav.attendanceFast`, `nav.attendanceSearch`.
...
- Follow-up (optional): remove legacy `/users`,`/permissions`,`/health`,`/scanner`,`/checkin`,`/reception` routes from `routes/index.tsx` once confirmed unused by operators.

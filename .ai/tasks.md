...
- Follow-up (optional): remove legacy `/users`,`/permissions`,`/health`,`/scanner`,`/reception`,`/packages`,`/cash` routes from `routes/index.tsx` once confirmed unused by operators. The daily-closing feature (الإغلاق اليومي) is fully removed (daily_closings tables, cash.daily_close perms, service, RPC, UI, dashboard); `docs/ai/*` (database.md, architecture.md, business-rules.md, roadmap.md) may still reference it and should be pruned in the next doc pass.

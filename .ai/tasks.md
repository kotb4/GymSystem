...
- Phase 7 (done, NOT yet committed): merged `subs`+`packages`+`plans` into one screen per user request. `/subscriptions` now has 3 tabs (الاشتراكات / الباقات / الخطط); `باقات` tab renders `<PackagesPage />` (which itself has الباقات + مقارنة) gated by `packages.view`. Removed `/packages` from `NAV_ROUTES` sidebar; legacy `/packages` route kept (bookmark compat). No backend/schema/permission changes (subscriptions reference `membership_plans` = خطط; الباقات is a separate richer catalog).

...
- Decision:
  ...
  8. **Backfill (migration v21).** The v21 callback inspects `expense_attachments` if it still exists; for every legacy row it (a) writes the BLOB bytes to `Files/expense_attachment/<id>.<ext>`, (b) inserts a corresponding row into `files` with `kind='expense_attachment'`, `sha256`, and the legacy `file_name`, (c) leaves `expense_attachments` intact for audit. The table is **not** dropped — operators can compare counts and drop it manually after a verification period. No destructive statement runs without an explicit log line.

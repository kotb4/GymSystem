# GymSystem Business Rules (verified from code)
...
- 68 permissions × 4 roles; owner always passes every check; other roles resolve from `role_permissions` table cached in memory (refreshed on boot and after each committed write); editable at runtime from Permissions page (requires `users.view` to view, `settings.edit` to mutate; owner row immutable).

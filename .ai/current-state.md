...
- `npm run e2e` (e2e-smoke) **now passes all 24 checks**; corrected the stale `unauthenticated /me` check in `scripts/e2e-smoke.ps1` — the server intentionally returns `200 {needsSetup:true}` (not 401) for unauthenticated `/api/auth/me`, so the old check asserted the opposite of the documented behavior.
- `scripts/e2e-audit.ps1` (42-check audit) is NOT fully green and this is **pre-existing/unrelated to TASK-021**: it logs `FAIL frozen subscription denied` (freezing one of a member's multiple active subs doesn't block scanning) and then aborts with a 400 on the member-photo fake upload (`/api/files?kind=member_photo`, line 276). Both are in features untouched by the treasury removal (freeze/subscriptions + file upload); left for a separate investigation.

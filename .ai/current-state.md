...
- **Current objective (done, this session):** **TASK-038 — remove the license grace period: lock EXACTLY at `expiresAt` with a TOTAL lockdown (ADR-022).** User decisions: at expiry = **قفل كامل تمامًا** (no read-only, no login — only the activation screen), keep the countdown banner. The earlier «اشيل التفعيل ازاي» question was dismissed — the protection system stays; only the grace behavior changed.
